...
async function downloadVideo(videoUrls, username, password, outputDirectory) {
   ...
   if(password === undefined) { // password not passed as argument
        ...
        if(argv.noKeyring === false) {
          try {
              await keytar.getPassword("UnibsDown", username).then(function(result) { password = result; });
              if (password === null) { // no previous password saved
                  password = await promptQuestion("Password not saved. Please enter your password, UnibsDown will not ask for it next time: ");
                  await keytar.setPassword("UnibsDown", username, password);
              } else {
   ...
   const usernameEmail = username + "@studenti.unibs.it";
   ...
   await page.$eval('input[id=userNameInput]', (el, value) => el.value = value, usernameEmail);
   await page.$eval('input[id=passwordInput]', (el, value) => el.value = value, password);
   await page.click('span[id="submitButton"]') // clicca sul tasto "Accedi"

   try {
     await page.waitForSelector('label[id="errorText"]', { timeout: 1000 });
   ...
   try {
	...
	} catch (error) { const data = await page.evaluate(() => document.querySelector('*').outerHTML);
   }
